...
export default function EditPortfolioItemPage({portfolioItem, portfolio}) {
  const [title, setTitle] = useState(portfolioItem.title)
  const [description, setDescription] = useState(portfolioItem.description)
  const [url, setUrl] = useState(portfolioItem.url)
  const [screenshot, setScreenshot] = useState(portfolioItem.thumbnail)
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true)
    try {
      await fetch(`/api/portfolios/${portfolio.id}/portfolio-items/${portfolioItem.id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
        ...
        if (res.ok) {
          toast.success('Updated portfolio item')
        }
      })
